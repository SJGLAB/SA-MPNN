...
class MegaScaleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split=None, csv_file=None):
        self.cfg = cfg
        self.split = split
        #use csv_file in local.yaml
        self.csv_file = csv_file


        df = pd.read_csv(self.csv_file, usecols=["ddG_ML", "mut_type", "WT_name", "aa_seq"])
        df = df.loc[df.ddG_ML != '-', :].reset_index(drop=True)
        df = df.loc[~df.mut_type.str.contains("ins") &
                    ~df.mut_type.str.contains("del") &
                    ~df.mut_type.str.contains(":"), :].reset_index(drop=True)
        self.df = df

        # WT_name in csv_file as wt_names
        self.wt_names = df["WT_name"].unique().tolist()
        self.wt_seqs = {}
        self.mut_rows = {}
