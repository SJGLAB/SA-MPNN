...
# 修复代码：将 PDB 复制匹配突变数量，并加入 Batch 防止显存溢出
            pred = []
            batch_size = 32  # 如果 GPU 显存富裕，可以调到 64 或 128 以加速推理
            
            for i in range(0, len(final_mutation_list), batch_size):
                batch_muts = final_mutation_list[i:i + batch_size]
                
                # 核心修复：把长度为 1 的 PDB 列表复制，使其与当前 batch 的突变数量严格一致
                batch_pdbs = mut_pdb * len(batch_muts)
                
                # 送入模型进行推理
                batch_pred = model(batch_pdbs, batch_muts)
                
                # 如果模型返回的是 tuple (如 pred, _)，只需提取真正的预测结果
